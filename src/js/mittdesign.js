...
function getItemTypeHtml(item) {
  ...
  switch (item.type) {
    ...
    case 'Discussion':
      html +=
        '<i class="icon-discussion" aria-label="Elementtype: diskusjoner"></i>';
      break;
    case 'Assignment':
      html +=
        '<i class="icon-assignment" aria-label="Elementtype: oppgave"></i>';
      break;
  }
  return html;
}
...
jQuery(function($) {
  ...
  if (myFrontPage.length) {
    mmooc.api.getModulesForCurrentCourse(function(modules) {
      ...
      if (myProgressbar.length) {
        ...
        html +=
          '<div class="progress bar bar-success" style="width:' +
          percentage +
          '%">';
        html += '</div>';
        html += '</div>';
  ...
  if (kuppModuleMenu.length) {
    mmooc.api.getCurrentModule(function(module) {
      ...
      html +=
        '<li class="module-home"><a href="/courses/"' +
        courseId +
        '"/modules/">Tilbake til moduloversikten</a></li>';
