describe('badges', function() {
  const name = 'Jasmine test',
    ...
    courseId = '1',
    completedText =
      'You have completed the modules required to achieve this badge.',
    mozilla = {
      url:
        'https://beta.matematikk.mooc.no/badgesafe/badge/40/4/299-znegva@reyraqguhar.pbz-assertion.json',
      name: 'Mozilla export',
  ...
  beforeEach(function() {
    generateTemplate = function(
      ...
      function addBackPackButton(backpackEnabled) {
        if (backpackEnabled) {
          return (
            '<a href="javascript: void(0);" class="claimbadge btn"' +
            'badge-url="' +
            mozilla.url +
            '"' +
            'badge-name="' +
      ...
      var template =
        ...
        '<a href="javascript:void(0)" onclick="openmypage(299,40,' +
        courseId +
        ',4); return false">' +
        '<img src="' +
        imgSrc +
        ...
        '</div>' +
        '<p id="badge-btn-22">' +
        addBackPackButton(backpackEnabled) +
        '</p>' +
        '<p>' +
        type +
        '</p>' +
  ...
  describe('backpack', function() {
    ...
    beforeEach(function() {
      template = jQuery(
        generateTemplate(name, type, imgSrc, courseId, completedText, true)
      );
    });
